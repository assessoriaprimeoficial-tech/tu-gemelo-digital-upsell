...
export function TuGemeloDigitalPage() {
  return <main className="upsell-font overflow-hidden bg-upsell-bg text-upsell-text selection:bg-upsell-blue selection:text-white">
    ...
    {/* SEÇÃO 1 — HERO */}
    <section className="relative overflow-hidden px-5 pb-12 pt-14 sm:px-6 sm:pb-16 sm:pt-20"><MatrixRain /><div className="pointer-events-none absolute left-1/2 top-0 h-[520px] w-[720px] -translate-x-1/2 rounded-full bg-upsell-blue/15 blur-[140px]" /><div className="relative mx-auto max-w-7xl"><div className="mx-auto max-w-5xl text-center"><span className="upsell-eyebrow inline-flex items-center gap-2 rounded-full border border-upsell-blue/25 bg-upsell-blue/10 px-4 py-2 text-[11px] tracking-[0.22em] text-upsell-blue uppercase sm:text-xs"><Sparkles className="h-4 w-4" /> Método Mente Expandida™</span><h1 className="mt-7 text-[2.5rem] font-bold leading-[1.06] text-white sm:text-6xl md:text-7xl">A IA já é genial.<span className="upsell-text-gradient-blue block">Falta ela saber<br className="hidden sm:block" /> quem é <span className="relative inline-block">você<span className="absolute -bottom-1 left-0 h-[6px] w-full rounded-full bg-upsell-blue/50 blur-[2px]" aria-hidden="true" /></span>.</span></h1><img src={heroCover.url} alt="Guia Tu Mente en GPT — transforme a IA em uma extensão da sua mente" className="mx-auto mt-8 w-full max-w-sm rounded-[24px] shadow-[0_30px_90px_-30px_rgba(96,165,250,0.45)] sm:max-w-md" /><p className="mx-auto mt-8 max-w-3xl text-base leading-7 text-upsell-text-muted sm:text-xl sm:leading-8">Pedir respostas genéricas qualquer um faz — e é exatamente por isso que não leva a nada. Quando a IA conhece quem você é, ela para de responder e <span className="font-semibold text-white">começa a criar com você, pelo você e do seu jeito.</span> O Método Mente Expandida constrói essa inteligência ao seu redor. Não é mais uma lista de prompts. É a sua vantagem injusta.</p><div className="mt-9 flex justify-center"><CTA /></div><div className="mt-5 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs font-semibold text-white/55 sm:text-sm"><span className="flex items-center gap-2"><CircleCheck className="h-4 w-4 text-upsell-success" /> Acesso digital imediato</span><span className="flex items-center gap-2"><LockKeyhole className="h-4 w-4 text-upsell-success" /> Pagamento 100% seguro</span><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-upsell-success" /> 7 dias de garantia</span></div></div></div></section>
